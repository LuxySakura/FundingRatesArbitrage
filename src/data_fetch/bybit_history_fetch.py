...
def bybit_fetch_history_funding_rates(symbol, segments, ticker, save_to_csv=True, csv_dir=None):
    ...
    if funding_rates_data:
        ...
        if 'bybitFR' in candle_data.columns:
            candle_data = candle_data.drop('bybitFR', axis=1)
        
        # 2. 创建一个新的DataFrame，用于存储合并后的数据
        merged_df = candle_data.copy()
        
        # 3. 初始化funding_rate列为NaN
        merged_df['bybitFR'] = float('nan')
        ...
        for _, row in funding_df.iterrows():
            ...
            if len(matching_indices) > 0:
                # 更新对应K线的资金费率
                merged_df.loc[matching_indices, 'bybitFR'] = row['funding_rate']
            else:
                logger.warning(f"未找到精确匹配的时间戳 {datetime.fromtimestamp(int(row['timestamp']) / 1000.0)} 对应的K线数据")
        
        logger.info(f"合并后共有 {len(merged_df)} 条记录，其中 {merged_df['bybitFR'].notna().sum()} 条有精确匹配的资金费率数据")
...
if __name__ == '__main__':
    ...
    bybit_fetch_history_funding_rates(symbol=symbol, segments=fr_segments)